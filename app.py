...
def chart_production_rates_per_well(wells_data, well, phases, smooth=True) -> go.Figure:
    C = get_C()
    phase_cfg = {
    ...
    for phase, (ph_col, dash, lbl) in phase_cfg.items():
        ...
        fig.add_trace(go.Scatter(
            x=df["Date"], y=y, name=lbl, mode="lines",
            line=dict(color=ph_col, dash=dash, width=2.2),
        ))
    fig.update_layout(
        title=dict(text=f"{well} Production Rates", font=dict(size=14, color=C["heading"])),
        showlegend=True, height=340,
        legend=dict(orientation="h", y=-0.22, font=dict(size=11, color=PBG["font"]["color"])),
...
def page_dashboard():
    ...
    _html('<div class="section-header">Production Rates by Well</div>')
    for well in selected:
        fig = chart_production_rates_per_well(wells_data, well, phases, smooth)
    ...
    er0 = eur_res.get(w0)
    if er0:
        fig_forecast = chart_cumulative_forecast(df0, er0, w0)
    else:
        cum = df0["Oil_bbl"].cumsum()
        fig_forecast = _themed_fig()
        fig_forecast.add_trace(go.Scatter(x=df0["Date"], y=cum/1e6, name="Cumulative Oil",
                                          mode="lines", line=dict(color=C["green"], width=2.5)))
        fig_forecast.update_layout(title=dict(text="Cumulative Oil Production", font=dict(size=14, color=C["heading"])),
                                   yaxis_title="MM STB", height=420,
                                   **get_plotly_bg())
    _html('<div class="section-header">Cumulative Forecast</div>')
    st.plotly_chart(fig_forecast, use_container_width=True)

    _html("<br>")
    _html('<div class="section-header">Latest Day Production by Phase</div>')
    phase_figs = chart_production_split_by_phase(wells_data, selected)
    cols = st.columns(3, gap="medium")
    for col, (phase_name, fig) in zip(cols, phase_figs.items()):
        with col:
            st.plotly_chart(fig, use_container_width=True)

    _html("<br>")
    _html('<div class="panel-card">')
    _html(f'<div class="section-header">Well Performance Summary</div>')
    rows = []
    for well in selected:
    ...
    st.dataframe(tbl, use_container_width=True, hide_index=True,
                 height=min(240, 36 * len(rows) + 38))
    _html('</div>')
    _html("<br>")
    _html(f'<div class="section-header">Insights & Alerts</div>')
    insights_html = ""
...
def page_dca():
    ...
    with right:
        _html('<div class="panel-card">')
        _html(f'<div class="section-header">Well Settings</div>')
        _html(result_row("Selected Well", well))
        _html(result_row("Field", st.session_state.field_name))
        _html(result_row("Forecast Horizon", f"{fore_yrs} yrs"))
        _html(result_row("Econ. Limit", f"{econ_lim:.0f} STB/d"))
        _html('</div>')
    ...
    if res_w:
        _html("<br>")
        _html('<div class="panel-card">')
        _html(f'<div class="section-header">All Model Fits (DCA Results)</div>')
        model_rows = []
    ...
    if dr:
        ...
        with left:
            _html('<div class="panel-card">')
            _html(f'<div class="section-header">Selected Model Summary</div>')
            rows_html = "".join([
        ...
        with right:
            ...
            _html('<div class="panel-card">')
            _html(f'<div class="section-header">Well Information</div>')
            info_html = "".join([
...
def page_eur():
    ...
    if not dr_all:
        ...
        return
    input_left, input_right = st.columns([2, 1], gap="medium")
    with input_left:
    ...
    with input_right:
        ...
        _html('<div class="panel-card">')
        _html(f'<div class="section-header">EUR Parameters</div>')
        _html(result_row("Selected Well", well))
        _html(result_row("Model", best_model(dr_all)))
        _html(result_row("Forecast Horizon", f"{horizon} yrs"))
        _html(result_row("Economic Limit", f"{econ_lim:.0f} STB/d"))
        _html('</div>')
    ...
    fig_forecast = chart_cumulative_forecast(df_w, eur, well)
    _html('<div class="section-header">Cumulative Forecast</div>')
    st.plotly_chart(fig_forecast, use_container_width=True)
    ...
    _html('<div class="section-header">Production Rate Forecast</div>')
    st.plotly_chart(fig_rate, use_container_width=True)

    _html(f'<div class="section-header">EUR Results (Base Case)</div>')
    ...
    res_data = {
        ...
        "Economic Limit": f"{eur['econ_limit']:.0f} STB/day",
